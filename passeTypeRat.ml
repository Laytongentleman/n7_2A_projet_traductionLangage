...
let rec analyse_type_expression e =
  ...
  | AstTds.AppelFonction (info, le) -> begin
    ...
    match info_ast_to_info info with
    | InfoFun (n, tret, tparams) ->
        (* Vérification du nombre de paramètres *)
        if List.length lte <> List.length tparams then
          raise (TypesParametresInattendus
                   (lte, List.map fst tparams))
        else begin
          (* Vérification type *)
          List.iter2
          (fun (_, arg_type) (param_type, _) ->
            if not (est_compatible arg_type param_type) then
              raise (TypesParametresInattendus (lte, List.map fst tparams));
          )
          (List.combine nle lte)
          tparams;
          ...
          (* vérification ref / non-ref *)
            List.iter2
              (fun arg (_, is_ref) ->
                 ...
                 | AstTds.Ref _, true -> ()
                 | AstTds.Ref _, false -> raise (ParametreNonRef n)
...
let analyse_type_fonction (AstTds.Fonction (tdecl, infofun, lp, li)) =
      ...
      List.iter2
        (fun (tdecl_param, _) (tparam, _is_ref) ->
           if not (est_compatible tdecl_param tparam) then
             raise (TypeInattendu (tdecl_param, tparam))
        )
        lp tparams;
  ...
  | _ -> failwith "analyse_type_fonction : identifiant non fonction"
